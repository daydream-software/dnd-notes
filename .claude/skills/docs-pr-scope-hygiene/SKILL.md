...
description: "Use when reviewing or preparing a documentation-focused PR to strip session-planning residue (plan.md, .squad/log) before merge."
metadata:
  version: 1.0.0
...
Use this when reviewing or preparing documentation-heavy PRs, especially agent-authored slices that may carry along execution artifacts. The goal is to keep repo history readable and avoid merging files that only exist to manage one session.
...
- Treat `.squad/log/` and `.squad/orchestration-log/` entries as runtime residue even if they were force-added once; if they are the only problem, remove just those files and avoid collateral edits to tracked coordination files.
...
- PR #78 hygiene follow-up: delete accidentally tracked `.squad/log/...` and `.squad/orchestration-log/...` files from the branch, but do not rewrite `.squad/identity/now.md` or inbox directives that belong to active coordination.
- `.squad/agents/{member}/history.md` can remain when it records durable team context for future sessions.
