...
Once the Postgres pod is running, create the databases the control-plane and tenant provisioner need:

```bash
kubectl --context dnd-notes-prod exec -n dnd-notes-platform platform-postgres-0 -- \
...
GHCR packages are currently public and do not require a pull secret. However, each package must be
linked to the `dnd-notes` repository before GitHub Actions can manage retention and before the package
shows up under the repo's package list. This is a one-time manual step per package.

After the first push to main lands an image set on GHCR:

1. Go to `https://github.com/orgs/daydream-software/packages`
2. For each of the four packages:
   - `dnd-notes`
...
This step becomes automatic once Issue #332 lands.
...
1. `pin-before-deploy` sentinel still in place — the tag was never substituted (Section 4).
   Run `kustomize edit set image` with a real tag and re-apply.
...
Patch the secret and restart the control-plane (same as Section 5d).
...
- **GHCR package linking is manual.** Until Issue #332 lands, each new package requires a one-time manual link to the repository (Section 6).
