...
- When a tag filter changes the visible note set, reconcile the selected note/editor state against that filtered list so the detail pane never points at a hidden record.
- Clear the active tag filter when the user explicitly starts a brand-new note, so creation does not inherit a browse-only slice and hide the new save from the main list.
...
- `apps/web/src/App.test.tsx` should prove that clicking a facet while a non-matching note is selected retargets or clears the editor instead of leaving a hidden note active.
...
- Filtering the list while leaving the editor/detail pane bound to a note outside the filtered result set.
