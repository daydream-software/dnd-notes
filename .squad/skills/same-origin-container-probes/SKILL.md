...
3. SPA fallbacks must only answer `GET`/`HEAD` navigation requests that actually accept HTML and do not look like file/asset fetches; let other requests fall through to 404/route handlers.
...
- `if (!request.accepts("html") || extname(request.path)) next()` before `index.html` fallback
