...
Team update (2026-05-16T00:00:00Z): When a test slice surfaces a production bug, open a tracking issue with patch sketch and expected test cases before merging the slice — the slice ships uncontaminated and the bug fix becomes a low-friction follow-up. Pattern proven by #308 (surfaced during #144 slice 7 review). — decided by Coordinator + Mikey (#308 fix)

Team update (2026-05-16T00:00:00Z): AGENTS.md branch-naming convention corrected — format is now `{type}/{kebab-case-slug}-{issue-number}` (e.g. `fix/clear-stale-tenant-display-name-248`), replacing stale `squad/{issue-number}-{kebab-case-slug}` form. Convention is empirical (anchored to real recent PRs), not aspirational — decided by Coordinator (chore/agents-md-copilot-drift)

Team update (2026-05-17T22:00:00Z): Phase 2 exit + repo hygiene sweep session completed. Coordinator triaged 4 PRs (PR #319 squad-skills migration + Copilot cleanup, PR #320 Keycloak-only auth cutover + test rewrite, PR #321 residual local-auth grep sweep, PR #322 share-link 429 root-cause fix). No Mikey local review gate needed — coordinator solo review + CodeRabbit covered all gates. 4 PRs merged. Issues #318 closed, #323 opened as follow-up. — decided by Coordinator
