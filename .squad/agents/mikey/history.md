...
- **Epic #42 Hygiene Sync — Phase 0 Execution Alignment (2026-04-19):** Audited GitHub issues against locked Phase 0 Postgres direction. Found three stale items: (1) issue #55 title still says "SQLite single-writer" despite Postgres pivot decision (2026-04-18); (2) issue #43 body says "blocked" despite hosting target being locked; (3) missing GitHub issue for Postgres adapter port (Track A, Phase 0 execution). Executed fixes immediately: retitled #55 to "Define tenant rolling-update and database connection-draining choreography (Postgres-backed)", unblocked #43 with clarifying comment on Phase 0 Track B scope, created new issue #58 "Port NoteStore adapter from SQLite to Postgres" assigned to squad:data. Added sync comment to epic #42 linking all three fixes. Decision record: `.squad/decisions/inbox/mikey-epic-hygiene.md`. **Key lesson:** GitHub epic sync is a standing practice after architectural pivots — stale issue titles and descriptions are the first source of downstream agent confusion. When decisions lock (especially pivots like SQLite → Postgres), audit the child issues same-day to catch title/scope misalignment. Issue #58 missing discovery would have left Phase 0 execution plan incomplete, with Track A implicitly lumped into #46 (SQL refactor), not recognized as a separate 5–7 day slice.

- **Epic #42 Wave 1 Priority Decision (2026-04-19):** Analyzed all five child issues (#52, #43, #53, #54, #55) for dependency structure and execution readiness. Decision: Wave 1 starts #52 (Brand, containerize) and #53 (Data, CP skeleton) in parallel on worktrees — these are the two load-bearing roots with zero blockers. Wave 2 (#43, #54, #55) waits on Wave 1. Found four stale items: (1) `now.md` references a Postgres adapter track mapped to closed #46, which was only the structural refactor — the actual Postgres port has no issue; (2) #55 title still says "SQLite single-writer" despite Postgres pivot; (3) #43 body still says "blocked" despite hosting being decided; (4) missing Postgres adapter issue for Phase 0. PRs must use `/copilot-review` and follow existing review gates. Decision: `.squad/decisions/inbox/mikey-epic-42-priority.md`. **Key lesson:** Always verify issue titles and bodies against locked decisions after an architectural pivot — the #55 SQLite title and the #46-as-Postgres-port reference in `now.md` would have sent agents down the wrong path.
