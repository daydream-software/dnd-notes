...
Team update (2026-05-16T00:00:00Z): jsdom location.assign workaround — use module-level Object.defineProperty(window, "location", { configurable: true, writable: true, value: { ...window.location, assign: assignMock } }) + mockReset in beforeEach; Vitest pool:threads isolates per file — decided by Chunk (#144 slice 2)
...
Team update (2026-05-16T00:00:00Z): Use `!= null` guards on `.json()` cast reads; runtime guards are load-bearing (TypeScript does not protect against missing fields at runtime for `response.json()`). When a test slice surfaces a production bug, open a tracking issue with patch sketch and expected test cases before merging the slice — the bug fix then lands as a low-friction mechanical follow-up. — decided by Chunk + Mikey (#308 fix)
