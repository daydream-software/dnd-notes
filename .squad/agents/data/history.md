...
- Postgres-backed tenant image rollouts now reuse `POST /internal/tenants/:tenantId/provision` with a version override; `apps/control-plane/src/provisioning.ts` marks ready tenants as `upgrading` during the rollout and returns them to `ready` once the new pod is Available.
- The generated tenant Deployment contract now explicitly stays single-replica `RollingUpdate` with `maxSurge: 1`, `maxUnavailable: 0`, `minReadySeconds: 5`, and `terminationGracePeriodSeconds: 30`; the operator choreography and connection-budget note live in `apps/control-plane/README.md` and `RUNTIME.md`.
