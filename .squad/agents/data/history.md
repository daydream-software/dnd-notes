...
📌 PR #108 follow-up completed (2026-04-24T13:27:40Z): Resolved all open review threads on PR #108 (Issue #100 centralized control-plane error logging). (1) Fixed bash-3.2 compatibility in smoke.sh: replaced bashism `${*: -1}` with Bash-3.2-safe `${!#}` pattern with guard check to avoid unset-variable error; smoke tests now portable across Bash 3.2+ systems. (2) Consolidated remaining ad-hoc error handlers in control-plane tenant routes (create/update/state/provision) to use centralized `getErrorMessage()` and `logUnexpectedError()` functions; reduces error-handling surface and improves observability. (3) Added focused regression coverage in app.test.ts (+56 lines) and k3d-smoke-payload.test.ts (+70 lines) for error paths and bash-compat logging. All control-plane tests passing (63/63); lint/build clean. Commit `611dbf6`. Orchestration log at `.squad/orchestration-log/2026-04-24T13-27-40Z-data-pr108.md`. Session log at `.squad/log/2026-04-24T13-27-40Z-pr108-followup.md`. Updated `.squad/skills/bash-compat/SKILL.md` with Bash-3.2 pattern reference. PR #108 ready for merge. GitHub smoke/validate workflows passing. — Data (Agent)

📌 PR #107 review follow-up completed (2026-04-24T13:13:26Z): Resolved all open review threads on PR #107 (Issue #101 tenant locking). Refactored tenant registry advisory-lock session management to pass checked-out Postgres client into provisioning callbacks, eliminating redundant lock re-acquisitions per callback. Added bounded `pg_try_advisory_lock()` retry with 5-second timeout and exponential backoff (50ms–300ms). Regression coverage: session reuse paths (5 assertions), lock timeout scenarios (2 test cases), concurrent lock attempt handling (3 edge cases). All control-plane tests passing (63/63); lint/build clean. Commit `8390705`. Orchestration log at `.squad/orchestration-log/2026-04-24_13-13-26Z-data.md`. Session log at `.squad/log/2026-04-24_13-13-26Z-pr107-review-followup.md`. PR #107 ready for merge. — Data (Agent)

📌 Issue #97 control-plane registry Postgres migration (2026-04-23T19:20:50Z): Migrated control-plane registry table from SQLite to Postgres as part of multi-agent delivery. Updated tests and environment documentation. Coordinated manifest changes with Brand and QA validation with Chunk. Orchestration logs at `.squad/orchestration-log/2026-04-23T19:20:50Z-data.md`. Session log at `.squad/log/2026-04-23T19:20:50Z-issue-97-control-plane-postgres.md`. All changes committed to worktree branch. Unit tests passing; smoke test pending Docker broker image resolution. — Data (Agent)
...
- Issue #97 control-plane registry is now Postgres-only: `apps/control-plane/src/tenant-registry-postgres.ts` owns the pooled registry implementation, `apps/control-plane/src/tenant-registry.ts` is just the thin Postgres delegator, and startup requires `CONTROL_PLANE_DATABASE_URL` in `apps/control-plane/src/index.ts`.
- Control-plane regression tests now use `apps/control-plane/test/tenant-registry-test-helpers.ts`, which provisions a `pg-mem` Pool and passes it into the registry so route/provisioning/auth suites can exercise the Postgres path without a live database.
