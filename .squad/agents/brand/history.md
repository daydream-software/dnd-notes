...
- **Mixed-runner CI reporting (2026-04-20):** For this monorepo, keep local `npm test` fail-fast, but add a separate root `npm run test:ci` orchestrator that always runs web, api, and control-plane suites, writes per-workspace JUnit XML into `reports/test-results/`, and then merges coverage outputs from `reports/coverage/*` into a single summary + `lcov.info`. That gives GitHub Actions one consolidated test-results check via `publish-unit-test-result-action`, while coverage stays visible in the job summary/artifact without inventing thresholds yet. Key files: `.github/workflows/ci.yml`, `scripts/run-ci-tests.mjs`, `scripts/merge-ci-coverage.mjs`, `apps/*/package.json`.
- **Root npm test exit contract (2026-04-20):** Keep the repo-level `npm test` path explicit in `package.json`: chain `test:web`, `test:api`, and `test:control-plane` instead of relying on npm workspace aggregation for the top-level failure contract. This repo mixes Vitest and Node's test runner, and the explicit `apps/*` entrypoints make exit-code behavior deterministic for local runs and CI while staying aligned with the existing monorepo script pattern.
- **Main rebase strategy (2026-04-19):** When rebasing local `main` onto `origin/main` with uncommitted changes, use `git stash` to preserve dirty files before rebase, then `git stash pop` after. This repo's rebase flow: stash (with message), rebase origin/main cleanly (no merge commits), pop stash to restore changes. All three local commits replayed cleanly on a04eaf7 without conflict. No need for interactive rebase or conflict resolution in a linear divergence.
- **Epic #42 Copilot PR flow audit (2026-04-19):** Confirmed worktree + Copilot review/automerge flow is production-ready. Worktree config correctly set in `.squad/config.json` with `workTreesFolder: ".worktrees"`, all branch filters (squad/* → main) active in both review and automerge workflows, CI runs on all PRs to main (including squad/* branches), merge gate enforces: PR open, not draft, squad/*, CI green, Copilot review on current SHA, no active threads. Merge trace recorded for audit trail. Found 6 minor sharp edges (branch naming not enforced, no visible branch protection rules check, squash-merge only, 5-min schedule trigger, GraphQL 100-thread limit, no explicit worktree mention in CI) — all acceptable and documented. Recommend proceeding with Issue #58 PR following existing pattern. See `.squad/decisions/inbox/brand-copilot-flow-audit.md`.
- **PR #60 squash-follow-up pattern (2026-04-19):** If a PR gets squash-merged before the Copilot loop is actually finished, do not try to resurrect the merged branch. Push any queued `main` updates first, leave a timeline note on the merged PR, then restart from fresh `main` in a new issue worktree/PR so the remaining review delta is small and audit-friendly.
- **PR #60 final Copilot blocker (2026-04-19):** Keep shutdown code aligned with the real entrypoint lifecycle. In this repo, `app.listen()` returns the server before signal handlers are registered, so a `const server` entrypoint should not carry an impossible `if (!server)` branch just to look defensive, and graceful termination should special-case benign `ERR_SERVER_NOT_RUNNING` close callbacks instead of converting normal duplicate/early shutdown paths into exit code `1`.
...
- **Gatekeeper merge trace notes (2026-04-19):** For GitHub-driven PR automerge, leave a concise conversation comment immediately before merge with a visible grep key plus a hidden head-SHA marker. That keeps post-mortem evidence on the PR itself, avoids duplicate spam on re-evaluation, and works best when the merge call is also pinned to the evaluated head SHA.
...
- **npm test tooling (2026-04-20):** Root `npm test` runs `npm run test --workspaces --if-present`, which correctly invokes web (vitest), api (node --test), and control-plane (node --test) in parallel. Root install must complete before workspace tests run; missing root dependencies cause "Cannot find package" errors on module resolution. Running `npm install` from root seeds all workspace `node_modules` via npm's symlink behavior, then tests pass cleanly. Test suite: 6 vitest files (32 tests) + 2 node test files (22 tests) = all 54 tests pass in ~16s.
📌 Team update (2026-04-20T13:31:33Z): npm-test-diagnosis complete — Root cause was missing `npm install` from repo root; fix applied; all workspace tests now pass; no code changes needed — Brand
