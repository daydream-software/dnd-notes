...
OPERATOR_PORTAL_IMAGE_REPOSITORY="${OPERATOR_PORTAL_IMAGE_REPOSITORY:-ghcr.io/daydream-software/dnd-notes-operator-portal}"
OPERATOR_PORTAL_IMAGE_TAG="${OPERATOR_PORTAL_IMAGE_TAG:-k3d}"
...
IMAGE_REF="${OPERATOR_PORTAL_IMAGE_REPOSITORY}:${OPERATOR_PORTAL_IMAGE_TAG}"

usage() {
  cat <<'EOF'
Build the operator-portal image and import it into k3d.

Environment overrides:
  K3D_CLUSTER_NAME                  Cluster name (default: dnd-notes)
  OPERATOR_PORTAL_IMAGE_REPOSITORY  Image repository (default: ghcr.io/daydream-software/dnd-notes-operator-portal)
  OPERATOR_PORTAL_IMAGE_TAG         Image tag (default: k3d)
...
echo "Building operator-portal image ${IMAGE_REF}..."
docker build --build-arg PORTAL_NAME=operator-portal -f "${ROOT}/docker/portal/Dockerfile" -t "${IMAGE_REF}" "${ROOT}"
...
echo "Operator-portal image ready in k3d: ${IMAGE_REF}"
