...
get_tenant_keycloak_access_token_pkce() {
  ...
  if [[ -z "${form_action}" ]]; then
    echo "Failed to extract Keycloak login form action from auth page" >&2
    return 1
  fi

  # Step 2: POST credentials to the form action — capture the Location redirect
  curl_with_mkcert_tls -sS \
    -c "${cookie_jar}" \
    -b "${cookie_jar}" \
    -X POST \
    -H 'Content-Type: application/x-www-form-urlencoded' \
  ...
  if [[ -z "${auth_code}" ]]; then
    echo "Failed to extract auth code from Keycloak redirect. Headers:" >&2
    cat "${headers_file}" >&2
    return 1
  fi
...
env \
  ...
  KEYCLOAK_ADMIN_BASE_URL="${CONTROL_PLANE_KEYCLOAK_URL}" \
  KEYCLOAK_ADMIN_REALM="${CONTROL_PLANE_KEYCLOAK_REALM}" \
  KEYCLOAK_ADMIN_CLIENT_ID=dnd-notes-keycloak-admin \
  KEYCLOAK_ADMIN_CLIENT_SECRET=dev-admin-client-secret \
  CUSTOMER_PORTAL_KEYCLOAK_URL="${CONTROL_PLANE_KEYCLOAK_URL}" \
  CUSTOMER_PORTAL_KEYCLOAK_REALM="${CONTROL_PLANE_KEYCLOAK_REALM}" \
...
tenant_redirect_uri="${TENANT_PUBLIC_SCHEME}://${tenant_subdomain}.${TENANT_BASE_DOMAIN}/auth-callback"
