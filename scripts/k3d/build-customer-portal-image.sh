...
CUSTOMER_PORTAL_IMAGE_REPOSITORY="${CUSTOMER_PORTAL_IMAGE_REPOSITORY:-ghcr.io/daydream-software/dnd-notes-customer-portal}"
CUSTOMER_PORTAL_IMAGE_TAG="${CUSTOMER_PORTAL_IMAGE_TAG:-k3d}"
...
IMAGE_REF="${CUSTOMER_PORTAL_IMAGE_REPOSITORY}:${CUSTOMER_PORTAL_IMAGE_TAG}"

usage() {
  cat <<'EOF'
Build the customer-portal image and import it into k3d.

Environment overrides:
  K3D_CLUSTER_NAME                  Cluster name (default: dnd-notes)
  CUSTOMER_PORTAL_IMAGE_REPOSITORY  Image repository (default: ghcr.io/daydream-software/dnd-notes-customer-portal)
  CUSTOMER_PORTAL_IMAGE_TAG         Image tag (default: k3d)
...
echo "Building customer-portal image ${IMAGE_REF}..."
docker build --build-arg PORTAL_NAME=customer-portal -f "${ROOT}/docker/portal/Dockerfile" -t "${IMAGE_REF}" "${ROOT}"
...
echo "Customer-portal image ready in k3d: ${IMAGE_REF}"
