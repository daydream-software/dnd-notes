...
Environment overrides:
  K3D_STATE_FILE
  K3D_OPERATOR_PORTAL_OVERRIDE_LOCAL_PORT
  K3D_OPERATOR_PORTAL_OVERRIDE_LISTEN_PORT
  K3D_OPERATOR_PORTAL_OVERRIDE_CHECK_ONLY=true
EOF
}
...
cleanup() {
  ...
  if (( exit_code == 0 )) && [[ "${CHECK_ONLY}" == "true" ]]; then
    ...
    log "Preserved operator-portal override logs in ${WORK_DIR}"
  fi
...
env_js="$(cat <<EOF
window.__ENV__ = {
  API_BASE_PATH: "/operator-api",
  KEYCLOAK_URL: "${keycloak_url}",
  KEYCLOAK_REALM: "${keycloak_realm}",
  KEYCLOAK_CLIENT_ID: "dnd-notes-control-plane"
};
EOF
)"

log "Starting local operator-portal..."
env \
  PORT="${LOCAL_PORT}" \
  VITE_OPERATOR_API_BASE_PATH="/operator-api" \
  VITE_OPERATOR_DEV_PROXY_TARGET="${control_plane_origin}" \
  npm run dev --workspace apps/operator-portal -- --port "${LOCAL_PORT}" \
  >"${WORK_DIR}/operator-portal.log" 2>&1 &
portal_pid=$!
...
env \
  ...
  K3D_PORTAL_OVERRIDE_K3D_API_ORIGIN="${control_plane_origin}" \
  K3D_PORTAL_OVERRIDE_LOCAL_VITE_ORIGIN="http://127.0.0.1:${LOCAL_PORT}" \
  K3D_PORTAL_OVERRIDE_API_PATH="/operator-api" \
...
tail -f "${WORK_DIR}/operator-portal.log" "${WORK_DIR}/portal-override-proxy.log"
