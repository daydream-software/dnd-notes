...
A freshly-bootstrapped, **idle** cluster (1 tenant provisioned, no traffic, no active Keycloak sessions, empty Postgres) sits at ~2.3 GiB of memory — `kubectl top nodes` typically shows ~1.4 GiB on the agent + ~0.9 GiB on the server, with Keycloak the largest pod at ~700 MiB. This is a floor, not a target: real load adds on top. Each active tenant API can run 150-300 MiB under traffic, Keycloak grows with active sessions, and Postgres scales with data volume and connection count. A multi-tenant dev session with the browser actively hitting the stack can easily reach 5-7 GiB.

`k3d:up` then layers parallel Docker builds on top of whatever the cluster needs, which can momentarily spike memory by several GiB. Plan for **10-12 GiB of host RAM** available to the dev environment during bootstrap, and budget **at least 8 GiB free** for normal day-to-day work after the cluster is running. WSL2 users should set `memory=10GB` or higher in `~/.wslconfig` as a blast-radius bound so a build spike can't OOM the whole host — this is a containment ceiling, not a sufficiency floor. Disk: budget ~5 GiB for the Postgres PVC, plus the Docker image cache which can grow well past 10 GiB across several rebuilds (clean with `docker system prune` if it bloats).
