...
- An in-process JWK cache and `getPublicKeyForKid(jwksUrl, kid)` loader that
  ...
  override behaviour)
- `verifyToken(rawJwt, { issuer, audience, jwksUrl, clockSkewSec?, notBeforeSkewSec? })`
  which enforces the RS256 alg allowlist, issuer/audience match (with the same
  Keycloak `azp`-takes-precedence semantics as before), strict-by-default `exp`
  checks, optional clock skew for `exp`, and an explicit `notBeforeSkewSec`
  override for `nbf` tolerance without relaxing expiration.
